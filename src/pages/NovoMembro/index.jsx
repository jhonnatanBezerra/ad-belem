...
export const NovoMembro = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('tipo_membro_id', selectedRule.id);
    data.append('congregacao_id',selectedCongregation.id);
    data.append('photo', selectedFile);

    {birthDate && data.append('nascimento', format(birthDate, 'yyyy-MM-dd HH:mm:ss'));}
    {baptismalDate && data.append('databatismo',format(baptismalDate, 'yyyy-MM-dd HH:mm:ss'));}
    {weddingDate && data.append('data_casamento', format(weddingDate, 'yyyy-MM-dd HH:mm:ss'));}
    {memberSince && data.append('membrodesde',format(memberSince, 'yyyy-MM-dd HH:mm:ss'));}
    {dateDelivered &&  data.append('cartao_dt_entreg',format(dateDelivered, 'yyyy-MM-dd HH:mm:ss'));}

    

    try {
      const {data: response, status} = await axios.post('http://127.0.0.1:8000/api/membros', data);
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={8}  sm={7}  md={4} >
              ...
              <FormControl required fullWidth>
                ...
                >
                 {listRole.map(role => (
                  <MenuItem key={role.id} value={role}>{role.tipo.toUpperCase()}</MenuItem>
                ))}
          ...
          <Grid item xs={6}  sm={6}  md={4} >
              <TextField 
                required
                name={'cpf'}
                value={helpers.maskDocuments(cpf)}
          ...
          <Grid item xs={6}  sm={6}  md={6} >
              <TextField 
                required
                name={'celular'}
                value={helpers.maskPhone(cellPhone)}
            ...
            <FormControl required fullWidth>
              ...
              >
                {listCongregation.map(congregation => (
                  ...
                  <MenuItem key={congregation.id}  value={congregation}> {congregation.congregacao.toUpperCase()}</MenuItem>
              ))}
          ...
          <Grid item xs={5} sm={3} md={3}  >
            <FormGroup>
              <FormControlLabel control={<Checkbox name={'suspenso'} value={suspended} onChange={()=>setSuspended(!suspended)} />} label="Suspenso" />
              <FormControlLabel control={<Checkbox name={'diciplinado'} value={disciplined} onChange={()=>setDisciplined(!disciplined)} />} label="Disciplinado" />
              <FormControlLabel control={<Checkbox name={'mudou'} value={moved} onChange={()=>setMoved(!moved)} />} label="Mudou" />
            </FormGroup>
          </Grid>
          ...
          <Grid item xs={12} sm={12} md={6}  >
            <Stack >
              <FormControlLabel control={<Checkbox name={'cartao_imp'} value={hasPrinted} onChange={()=>setHasPrinted(!hasPrinted)}  />} label="Impresso" />
              
              <Stack direction={'row'} >
               <FormControlLabel control={<Checkbox name={'cartao_entregue'} value={hasDelivered} onChange={()=>setHasDelivered(!hasDelivered)}  />} label="Entregue" />
                <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={brLocale}>
                        <DatePicker
